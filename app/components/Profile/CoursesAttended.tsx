...
const CoursesAttended: FC<Props> = ({ user }) => {
    ...
    return (
        <div className="text-white px-4 py-6">
            <h2 className="text-2xl font-semibold mb-4">Khóa học đã tham gia</h2>
            {courses.length > 0 ? (
                        ...
                        <div key={course?._id} className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex justify-between items-center">
                                    <h4 className="text-lg font-semibold text-primary dark:text-yellow-400">
                                        {course.price === 0 ? "Miễn phí" : formatVNDPrice(course.price)}
                                    </h4>
                                    <Link href={`/course/${course._id}`} passHref>
                                        <p className="text-blue-500 hover:underline text-sm">Xem chi tiết</p>
                                    </Link>
                                </div>
                            </div>
                        </div>
